var prefix = "/back/api/undercarriage"
var goodsLabel="1";
$(function() {
	load();
	goodsTypeOption();
...
function edit(id) {
	var editPage = layer.open({
		...
		area : [ '80%', '80%' ],
		content : '/back/undercarriage/edit?storeGoodsId=' + id // iframe的url
	});
	layer.full(editPage);
}
function remove(id) {
	layer.confirm('确定要删除选中的记录？', {
		...
		$.ajax({
			url :"/back/api/undercarriage/delete",
			type : "post",
			data : {
				'id' : id
			},
...
function batchRemove() {
	...
	// 按钮
		...
		$.ajax({
			...
			url : '/back/api/undercarriage/batchRemove',
			success : function(r) {
				if (r.ret =='0') {
					layer.msg(r.msg);
					reLoad();
